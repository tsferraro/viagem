...
⛔ **`mapsQuery` é obrigatório nas duas estruturas que montam ROTA**: parada de
**walking tour** e ponto de **road trip** (dia com `transport: "driving"`, o que a skill
`road-trip-designer` gera). `validate.py` **bloqueia** (`check_wt_maps_query`). Dia comum de
rota só recebe aviso — ganha nome quando alguém preencher, sem travar a entrega. E preencher não basta: cada query tem que ser
...
- **★ no Resumo = atração de maior recompensa do dia** (`renderOverview` · corrigido 2026-08-04): a linha `★` de cada dia mostra o card com maior `valeAPena` (empate → o mais cedo). ⛔ **Nunca `cards[0]`** — o primeiro stop costuma ser logística, e no roteiro dos pais isso fazia 7 dos 16 dias anunciarem "Check-out + saída cedo" ou "Café da manhã" como a atração. `validate.py` guarda a assinatura.
